...
class cell {
    public:
        ...
        cell(C const &cell_id, S initial_state, vector<C> const &neighbors, string const &delayer_id, Args&&... args) {
            NV vicinity = NV();
         ...
         */
        template <typename... Args>
        cell(C const &cell_id_in, S initial_state, NV const &vicinity, string const &delayer_id, Args&&... args) {
        ...
        virtual T output_delay(S const &cell_state) const { return numeric_limits<T>::infinity(); }
        ...
        typename make_message_bags<output_ports>::type output() const {
            ...
            vector<cell_state_message<C, S>> bag_port_out = {cell_state_message<C, S>(cell_id, public_state)};
        ...
        friend std::ostringstream& operator << (std::ostringstream& os, const typename cell<T, C, S, V, C_HASH>::state_type& i) {
            os << i.current_state;
            return os;
        }
};
