...
template <typename T>
class default_cell: public cell<T, int, int> {
public:
    using cell<T, int, int>::cell_id;
    using cell<T, int, int>::state;

    explicit default_cell() : cell<T, int, int>() {}

    template <typename... Args>
    default_cell(int cell_id, int initial_state, std::vector<int> const &neighbors, std::string const &delayer_id, Args&&... args):
            cell<T, int, int>(cell_id, initial_state, neighbors, delayer_id, std::forward<Args>(args)...) {}
    ...
    default_cell(int cell_id, int initial_state, std::unordered_map<int, int> const &vicinities, std::string const &delayer_id, Args&&... args):
            cell<T, int, int>(cell_id, initial_state, vicinities, delayer_id, std::forward<Args>(args)...) {}
